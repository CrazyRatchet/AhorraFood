...
export default function RegisterStep2() {
  ...
  const handleNext = () => {
    ...
    if (!timeRegex.test(horaApertura)) {
      Alert.alert(
        "Error",
        "Formato de hora de apertura inválido. Usa formato HH:MM (ej. 08:00)"
      );
      return;
    }
    ...
    if (!timeRegex.test(horaCierre)) {
      Alert.alert(
        "Error",
        "Formato de hora de cierre inválido. Usa formato HH:MM (ej. 18:00)"
      );
      return;
    }
          ...
          <View style={styles.optionsRow}>
            {["Fonda", "Supermercado"].map((cat) => (
              <TouchableOpacity
                ...
                style={[
                  styles.option,
                  form.categoria === cat && styles.selectedOption,
                ]}
